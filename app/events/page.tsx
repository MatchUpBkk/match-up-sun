...
export default function EventsPage() {
  ...
  const results = useMemo(() => applyFilters(sampleEvents, filters), [filters]);

  return (
    <div className="container-x pt-28 pb-24 sm:pt-32">
      <Reveal>
        <span className="chip mb-4">{t('discover.featured')}</span>
        <h1 className="text-4xl font-extrabold text-white sm:text-5xl">{t('nav.events')}</h1>
        <p className="mt-3 max-w-2xl text-white/60">{t('discover.subtitle')}</p>
      </Reveal>
